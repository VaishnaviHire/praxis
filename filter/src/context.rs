...
impl HttpFilterContext<'_> {
    ...
    /// Correlation ID: request-scoped [`TraceContext`] if initialized,
    /// otherwise an inbound `x-request-id`.
    ///
    ...
    /// Inject hop correlation from request-scoped [`TraceContext`].
    ///
    /// No-op when `trace_context` has not run.
    ///
    /// [`TraceContext`]: crate::trace_context::TraceContext
    ...
    ///
    /// When [`TraceContext`] is present, injects `x-request-id` and a
    /// hop `traceparent` via [`FrameworkHeaders`]. Callers do not set
    /// those headers manually.
    ///
    /// [`TraceContext`]: crate::trace_context::TraceContext
    ...
    /// # Errors
    ///
    /// Returns [`SubRequestError`] when the sub-request client is
    /// unavailable or the transport exchange fails.
    ///
    ...
    ///
    /// Same automatic correlation as [`execute_subrequest`](Self::execute_subrequest).
    ///
    /// [`TraceContext`]: crate::trace_context::TraceContext
    ///
    /// # Errors
