...
//! Request-scoped correlation for forwarded requests and sub-requests.
//!
//! Each outbound hop mints its own span-id under a shared trace-id so
//! legs stay jointly correlatable and individually measurable.
...
/// Correlation identifiers resolved once per downstream request.
///
/// Outbound hops share [`trace_id`](Self::trace_id) and mint a fresh span-id.
...
impl TraceContext {
    ...
    /// Inject correlation headers for one outbound hop.
    ///
    /// # Errors
    ///
    /// Returns [`SubRequestError::InvalidRequest`] when a header value is invalid.
...
///
/// Returns `None` for malformed values, all-zero ids, or version `ff`.
/// Higher versions may append fields after flags; those are ignored and
/// the base fields are continued. Version `00` rejects trailing fields.
#[must_use]
...
/// Last 16 hex chars of the generator id: the first 16 are timestamp-dominated
/// and collide for every id minted in the same microsecond.
