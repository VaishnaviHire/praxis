...
| `reject_oversized` | bool | no | Reject a body sized exactly at the inspection buffer limit (1 MiB) instead of inspecting it. Bodies that exceed the limit are always rejected with 413 by the streaming buffer regardless of this flag; there is no silent truncation. |
